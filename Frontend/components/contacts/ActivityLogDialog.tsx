...
export function ActivityLogDialog({ accountId, contactId, onOpenChange, open }: ActivityLogDialogProps) {
  ...
  const createActivity = useMutation({
    ...
    onSuccess: () => {
      form.reset();
      void queryClient.invalidateQueries({ queryKey: ["contacts", "timeline", contactId] });
    ...
    <Dialog onOpenChange={onOpenChange} open={open}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Add a call, email, meeting, or note to this contact.</DialogDescription>
        </DialogHeader>
        <form className="grid gap-4" onSubmit={form.handleSubmit((values) => createActivity.mutate(values))}>
          ...
          <div>
            ...
            <Input id="activity_subject" {...form.register("subject")} />
            {fieldError(form.formState.errors.subject?.message)}
          </div>
          <div>
            <Label htmlFor="activity_body">Body</Label>
            <textarea
          ...
          <div>
            ...
            <Input id="activity_outcome" {...form.register("outcome")} />
          </div>
